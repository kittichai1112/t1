...
try:
    # Try to import OrangePi.GPIO (if available)
    import OrangePi.GPIO as GPIO
    GPIO_AVAILABLE = True
    print("Using OrangePi.GPIO")
except ImportError:
    try:
        # Try to use gpiod (modern Linux GPIO interface)
        import gpiod
        GPIO_AVAILABLE = True
        GPIO_TYPE = 'gpiod'
        print("Using gpiod library")
    except ImportError:
        # Fallback to manual file-based GPIO control
        GPIO_AVAILABLE = False
        GPIO_TYPE = 'manual'
        print("Using manual GPIO control (no GPIO library found)")

class OrangePiGPIO:
    """GPIO wrapper for Orange Pi that handles different GPIO backends"""
    ...
    def __init__(self):
        ...
        if GPIO_AVAILABLE:
            try:
                import OrangePi.GPIO as GPIO
                self.gpio = GPIO
                self.gpio_type = 'orangepi'
                print("Initialized OrangePi.GPIO")
            except ImportError:
                try:
                    import gpiod
                    self.chip = gpiod.Chip('gpiochip0')  # Usually gpiochip0 on Orange Pi
                    self.gpio_type = 'gpiod'
                    print("Initialized gpiod")
                except Exception as e:
                    print(f"gpiod initialization failed: {e}")
                    self.gpio_type = 'manual'
        else:
            self.gpio_type = 'manual'
            print("Using manual GPIO control")
    
    def setmode(self, mode):
        if self.gpio_type == 'orangepi':
    ...
    def setup(self, pin, direction, initial=None, pull_up_down=None):
        if self.gpio_type == 'orangepi':
            if initial:
                ...
                self.gpio.setup(pin, direction, pull_up_down=pull_up_down)
            else:
            ...
            try:
                if direction == self.OUTPUT:
                    ...
                    line.request(consumer="bgt60tr13c", type=gpiod.LINE_REQ_DIR_OUT)
                    if initial is not None:
                        line.set_value(1 if initial else 0)
                    self.lines[pin] = line
                elif direction == self.INPUT:
                    line = self.chip.get_line(pin)
                    line.request(consumer="bgt60tr13c", type=gpiod.LINE_REQ_DIR_IN)
                    self.lines[pin] = line
            except Exception as e:
                print(f"GPIO setup failed for pin {pin}: {e}")
        elif self.gpio_type == 'manual':
            # Manual GPIO setup via sysfs
            try:
                # Export GPIO
                ...
                # Set direction
                direction_str = 'out' if direction == self.OUTPUT else 'in'
                with open(f'/sys/class/gpio/gpio{pin}/direction', 'w') as f:
                    f.write(direction_str)
                
                # Set initial value if output
                if direction == self.OUTPUT and initial is not None:
                    with open(f'/sys/class/gpio/gpio{pin}/value', 'w') as f:
                        f.write('1' if initial else '0')
                        
            except Exception as e:
                print(f"Manual GPIO setup failed for pin {pin}: {e}")
    ...
    def output(self, pin, value):
        if self.gpio_type == 'orangepi':
            ...
            try:
                ...
                print(f"Manual GPIO output failed for pin {pin}: {e}")
    ...
    def input(self, pin):
        if self.gpio_type == 'orangepi':
            ...
            try:
                ...
                print(f"Manual GPIO input failed for pin {pin}: {e}")
                return 0
        return 0
    ...
    def cleanup(self):
        if self.gpio_type == 'orangepi':
            ...
            # Unexport all used GPIOs
            for pin in self.lines.keys():
                try:
    ...
    @property
    def BCM(self):
        return getattr(self.gpio, 'BCM', 11) if self.gpio_type == 'orangepi' else 11
    
    @property
    def OUT(self):
        return getattr(self.gpio, 'OUT', 1) if self.gpio_type == 'orangepi' else 1
    ...
    @property
    def IN(self):
        return getattr(self.gpio, 'IN', 0) if self.gpio_type == 'orangepi' else 0
    ...
    @property
    def HIGH(self):
        return getattr(self.gpio, 'HIGH', 1) if self.gpio_type == 'orangepi' else 1
    
    @property
    def LOW(self):
        return getattr(self.gpio, 'LOW', 0) if self.gpio_type == 'orangepi' else 0
    
    @property
    def PUD_UP(self):
        return getattr(self.gpio, 'PUD_UP', 2) if self.gpio_type == 'orangepi' else 2

class BGT60TR13C:
    def __init__(self, spi_bus=1, spi_device=0, cs_pin=8, rst_pin=7, irq_pin=25,
        ...
        self.spi_bus = spi_bus  # Orange Pi Zero 2 typically uses spidev1.0
    ...
    def begin(self):
        ...
        try:
            ...
            if self.irq_pin is not None:
                self.GPIO.setup(self.irq_pin, self.GPIO.IN, pull_up_down=self.GPIO.PUD_UP)
...
if __name__ == "__main__":
    # Create sensor instance (using SPI bus 1 for Orange Pi Zero 2)
    sensor = BGT60TR13C(spi_bus=1, spi_device=0)  # Orange Pi Zero 2 uses spidev1.0
